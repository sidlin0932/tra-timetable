...
<body>
    ...
    <header class="navbar">
        <div class="nav-brand">
            ...
            <div class="nav-title">
                <h1>台鐵全路網時刻表 & 跨區間智慧轉乘導航</h1>
                <p>2026/07/01 大改點全線全班次 · 支援主幹線與四大支線多階複合排序</p>
            </div>
        </div>
    ...
    <div class="container">
            ...
            <div class="query-grid">
                <div class="form-group">
                    ...
                    <div class="station-input-box">
                        <input type="text" id="originInput" class="station-input" value="內灣" placeholder="輸入站名 (如: 內灣、台北、台中)..." autocomplete="off">
                        <button class="btn-station-picker" onclick="openStationModal('origin')">📋 車站選單</button>
                ...
                <div class="form-group">
                    ...
                    <div class="station-input-box">
                        <input type="text" id="destInput" class="station-input" value="平溪" placeholder="輸入站名 (如: 平溪、花蓮、高雄)..." autocomplete="off">
                        <button class="btn-station-picker" onclick="openStationModal('dest')">📋 車站選單</button>
            ...
            <div class="filter-row">
                <div class="form-group">
                    ...
                    <div class="segmented-control" id="transferFilter">
                        <button class="segment-btn active" onclick="setTransferCondition('all', this)">接受轉乘 (推薦)</button>
        ...
        <section class="sort-bar" id="sortBar">
            <div class="sort-summary">
                ...
                <span id="routeSummaryText" style="color: var(--primary); font-weight: 700;">內灣 ➔ 平溪</span>
            </div>
    ...
    <script>
        ...
        const STATION_GROUPS = [
            {
                line: '支線 (內灣 / 平溪 / 集集 / 沙崙)',
                stations: ['內灣', '富貴', '合興', '九讚頭', '橫山', '竹東', '榮華', '上員', '六家', '竹中', '新莊', '千甲', '平溪', '菁桐', '嶺腳', '望古', '十分', '大華', '三貂嶺', '猴硐', '海科館', '八斗子', '車埕', '水里', '集集', '龍泉', '濁水', '源泉', '沙崙', '長榮大學']
            },
            {
                line: '西部幹線 (北部：基隆 ➔ 新竹)',
                stations: ['基隆', '三坑', '八堵', '七堵', '百福', '五堵', '汐止', '汐科', '南港', '松山', '台北', '萬華', '板橋', '浮洲', '樹林', '南樹林', '山佳', '鶯歌', '桃園', '內壢', '中壢', '埔心', '楊梅', '富岡', '新富', '北湖', '湖口', '新豐', '竹北', '北新竹', '新竹']
            },
            {
                line: '西部幹線 (台中線山線：新竹 ➔ 彰化)',
                stations: ['三姓橋', '香山', '崎頂', '竹南', '造橋', '豐富', '苗栗', '南勢', '銅鑼', '三義', '泰安', '后里', '豐原', '栗林', '潭子', '頭家厝', '松竹', '太原', '精武', '台中', '五權', '大慶', '烏日', '新烏日', '成功', '彰化']
            },
            {
                line: '西部幹線 (海岸線海線：竹南 ➔ 彰化)',
                stations: ['談文', '大山', '後龍', '龍港', '白沙屯', '新埔', '通霄', '苑裡', '日南', '大甲', '台中港', '清水', '沙鹿', '龍井', '大肚', '追分']
            },
            {
                line: '西部幹線 (南部：彰化 ➔ 高雄)',
                stations: ['花壇', '大村', '員林', '永靖', '社頭', '田中', '二水', '林內', '石榴', '斗六', '斗南', '石龜', '大林', '民雄', '嘉北', '嘉義', '水上', '南靖', '後壁', '新營', '柳營', '林鳳營', '隆田', '拔林', '善化', '南科', '新市', '永康', '大橋', '台南', '保安', '仁德', '中洲', '大湖', '路竹', '岡山', '橋頭', '楠梓', '新左營', '左營', '內惟', '美術館', '鼓山', '三塊厝', '高雄']
            },
            {
                line: '屏東線 (高雄 ➔ 枋寮)',
                stations: ['民族', '科工館', '正義', '鳳山', '後庄', '九曲堂', '六塊厝', '屏東', '歸來', '麟洛', '西勢', '竹田', '潮州', '崁頂', '南州', '鎮安', '林邊', '佳冬', '東海', '枋寮']
            },
            {
                line: '東部幹線 (宜蘭線：八堵 ➔ 蘇澳)',
                stations: ['暖暖', '四腳亭', '瑞芳', '牡丹', '雙溪', '貢寮', '福隆', '石城', '大里', '大溪', '龜山', '外澳', '頭城', '頂埔', '礁溪', '四城', '宜蘭', '二結', '中里', '羅東', '冬山', '新馬', '蘇澳新', '蘇澳']
            },
            {
                line: '東部幹線 (北迴線：蘇澳新 ➔ 花蓮)',
                stations: ['永樂', '東澳', '南澳', '武塔', '漢本', '和平', '和仁', '崇德', '新城(太魯閣)', '景美', '北埔', '花蓮']
            },
            {
                line: '東部幹線 (台東線：花蓮 ➔ 台東)',
                stations: ['吉安', '志學', '平和', '壽豐', '豐田', '林榮新光', '南平', '鳳林', '萬榮', '光復', '大富', '富源', '瑞穗', '三民', '玉里', '東里', '東竹', '富里', '池上', '海端', '關山', '月美', '瑞和', '瑞源', '鹿野', '山里', '台東']
            },
            {
                line: '南迴線 (枋寮 ➔ 台東)',
                stations: ['加祿', '內獅', '枋山', '大武', '瀧溪', '金崙', '太麻里', '知本', '康樂']
            }
        ];

        const ALL_STATIONS = Array.from(new Set(STATION_GROUPS.flatMap(g => g.stations)));
        ...
        function planRoutes(orig, dest, startTimeMin) {
            ...
            for (const t of allTimetableData) {
                if (typeFilter === 'trpass' && !t.is_trpass) continue;
                if (typeFilter === 'express' && !['自強號', '新自強(EMU3000)', '普悠瑪', '太魯閣', '莒光號'].includes(t.train_type)) continue;
                if (typeFilter === 'local' && !['區間車', '區間快'].includes(t.train_type)) continue;

                let idxO = -1;
                let idxD = -1;
                for (let i = 0; i < t.stops.length; i++) {
                    if (t.stops[i].station === orig && idxO === -1) idxO = i;
                    if (t.stops[i].station === dest && idxO !== -1) { idxD = i; break; }
                }

                if (idxO !== -1 && idxD !== -1) {
                    const dep = timeToMin(t.stops[idxO].time);
                    const arr = timeToMin(t.stops[idxD].time);
                    if (dep >= startTimeMin && arr > dep) {
                        results.push({
                            transfers: 0,
                            dep_time: t.stops[idxO].time,
                            arr_time: t.stops[idxD].time,
                            duration: arr - dep,
                            is_trpass: t.is_trpass,
                            train_types: [t.train_type],
                            legs: [{
                                ...
                                from: orig,
                                to: dest,
                                dep: t.stops[idxO].time,
                                arr: t.stops[idxD].time,
                                all_stops: t.stops.slice(idxO, idxD + 1)
                            }]
                        });
                    }
                }
            ...
            if (transferCondition === 'direct') {
                return results;
            }

            // 2. 1-Transfer (A ➔ T1 ➔ B)
            const trainsFromOrig = [];
            for (const t of allTimetableData) {
                if (typeFilter === 'trpass' && !t.is_trpass) continue;
                for (let i = 0; i < t.stops.length - 1; i++) {
                    if (t.stops[i].station === orig) {
                        const depMin = timeToMin(t.stops[i].time);
                        if (depMin >= startTimeMin) {
                            for (let j = i + 1; j < t.stops.length; j++) {
                                trainsFromOrig.push({
                                    train: t,
                                    depTime: t.stops[i].time,
                                    arrSt: t.stops[j].station,
                                    arrTime: t.stops[j].time,
                                    subStops: t.stops.slice(i, j + 1)
                                });
                            }
                        }
                    }
            ...
            for (const t of allTimetableData) {
                if (typeFilter === 'trpass' && !t.is_trpass) continue;
                for (let j = 1; j < t.stops.length; j++) {
                    if (t.stops[j].station === dest) {
                        for (let i = 0; i < j; i++) {
                            trainsToDest.push({
                                train: t,
                                depSt: t.stops[i].station,
                                depTime: t.stops[i].time,
                                arrTime: t.stops[j].time,
                                subStops: t.stops.slice(i, j + 1)
                            });
            ...
            for (const leg1 of trainsFromOrig) {
                const arr1Min = timeToMin(leg1.arrTime);
                for (const leg2 of trainsToDest) {
                    if (leg1.arrSt === leg2.depSt && leg1.train.train_number !== leg2.train.train_number) {
                        const dep2Min = timeToMin(leg2.depTime);
                        const layover = dep2Min - arr1Min;
                        if (layover >= 3 && layover <= 120) {
                            const totalDur = timeToMin(leg2.arrTime) - timeToMin(leg1.depTime);
                            results.push({
                                transfers: 1,
                                dep_time: leg1.depTime,
                                arr_time: leg2.arrTime,
                                duration: totalDur,
                                is_trpass: leg1.train.is_trpass && leg2.train.is_trpass,
                                train_types: [leg1.train.train_type, leg2.train.train_type],
                                transfer_stations: [leg1.arrSt],
                                legs: [
                                    {
                                        train_number: leg1.train.train_number,
                                        train_type: leg1.train.train_type,
                                        train_model: leg1.train.train_model,
                                        is_trpass: leg1.train.is_trpass,
                                        origin: leg1.train.origin,
                                        dest: leg1.train.dest,
                                        from: orig,
                                        to: leg1.arrSt,
                                        dep: leg1.depTime,
                                        arr: leg1.arrTime,
                                        all_stops: leg1.subStops
                                    },
                                    {
                                        train_number: leg2.train.train_number,
                                        train_type: leg2.train.train_type,
                                        train_model: leg2.train.train_model,
                                        is_trpass: leg2.train.is_trpass,
                                        origin: leg2.train.origin,
                                        dest: leg2.train.dest,
                                        from: leg2.depSt,
                                        to: dest,
                                        dep: leg2.depTime,
                                        arr: leg2.arrTime,
                                        layover: layover,
                                        all_stops: leg2.subStops
                                    }
                                ]
                            });
                        }
            ...
            // 3. 2-Transfers (A ➔ H1 ➔ H2 ➔ B)
            if (transferCondition === 'all') {
                const keyHubs = new Set([
                    '新竹', '北新竹', '竹中', '樹林', '板橋', '台北', '松山', '南港',
                    '七堵', '八堵', '瑞芳', '宜蘭', '羅東', '花蓮', '二水', '彰化',
                    '台中', '嘉義', '台南', '新左營', '高雄', '屏東', '潮州', '枋寮', '台東'
                ]);

                for (const leg1 of trainsFromOrig) {
                    if (!keyHubs.has(leg1.arrSt)) continue;
                    const arr1Min = timeToMin(leg1.arrTime);

                    for (const leg3 of trainsToDest) {
                        if (!keyHubs.has(leg3.depSt) || leg1.arrSt === leg3.depSt) continue;
                        const dep3Min = timeToMin(leg3.depTime);
                        if (dep3Min <= arr1Min + 15) continue;

                        const h1 = leg1.arrSt;
                        const h2 = leg3.depSt;

                        for (const t2 of allTimetableData) {
                            if (typeFilter === 'trpass' && !t2.is_trpass) continue;
                            let idxH1 = -1;
                            let idxH2 = -1;
                            for (let k = 0; k < t2.stops.length; k++) {
                                if (t2.stops[k].station === h1 && idxH1 === -1) idxH1 = k;
                                if (t2.stops[k].station === h2 && idxH1 !== -1) { idxH2 = k; break; }
                            }

                            if (idxH1 !== -1 && idxH2 !== -1) {
                                const dep2Min = timeToMin(t2.stops[idxH1].time);
                                const arr2Min = timeToMin(t2.stops[idxH2].time);
                                const layover1 = dep2Min - arr1Min;
                                const layover2 = dep3Min - arr2Min;

                                if (layover1 >= 3 && layover1 <= 80 && layover2 >= 3 && layover2 <= 80) {
                                    const totalDur = timeToMin(leg3.arrTime) - timeToMin(leg1.depTime);
                                    results.push({
                                        transfers: 2,
                                        dep_time: leg1.depTime,
                                        arr_time: leg3.arrTime,
                                        duration: totalDur,
                                        is_trpass: leg1.train.is_trpass && t2.is_trpass && leg3.train.is_trpass,
                                        train_types: [leg1.train.train_type, t2.train.train_type, leg3.train.train_type],
                                        transfer_stations: [h1, h2],
                                        legs: [
                                            {
                                                train_number: leg1.train.train_number,
                                                train_type: leg1.train.train_type,
                                            ...
                                            },
                                            {
                                                ...
                                                is_trpass: t2.is_trpass,
                                                origin: t2.origin,
                                                dest: t2.dest,
                                                from: h1,
                                                to: h2,
                                                dep: t2.stops[idxH1].time,
                                                arr: t2.stops[idxH2].time,
                                                layover: layover1,
                                                all_stops: t2.stops.slice(idxH1, idxH2 + 1)
                                            },
                                            {
                                                train_number: leg3.train.train_number,
                                                train_type: leg3.train.train_type,
                                                train_model: leg3.train.train_model,
                                                is_trpass: leg3.train.is_trpass,
                                                origin: leg3.train.origin,
                                                dest: leg3.train.dest,
                                                from: h2,
                                                to: dest,
                                                dep: leg3.depTime,
                                                arr: leg3.arrTime,
                                                layover: layover2,
                                                all_stops: leg3.subStops
                                            }
                                        ]
                                    });
                                }
            ...
            if (currentRoutes.length === 0) {
                container.innerHTML = `
                    <div class="empty-state">
                        <h3>🔍 查無符合條件的列車乘車方案</h3>
                        <p>建議調整出發時間、放寬轉乘條件，或確認起訖站點是否正確。</p>
                    </div>
                `;
                return;
            }
        ...
        function openStationModal(type) {
            ...
            document.getElementById('modalTitle').textContent = `選擇${type === 'origin' ? '出發' : '抵達'}車站`;
        ...
        function renderStationModal() {
            ...
            body.innerHTML = STATION_GROUPS.map(group => `
                <div class="line-section">
                    <div class="line-section-title">📍 ${group.line}</div>
...
target_path = 'f:/Antigravity/台鐵時刻表0701/index.html'
