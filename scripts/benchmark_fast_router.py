...
def ultra_fast_plan_routes(orig, dest, startTimeMin):
    t0 = time.time()
    orig_deps = deps_by_st.get(orig, [])
    direct_results = []
    transfer_results = []
    
    # 1. Direct trains across full day (instant loop)
    for d1 in orig_deps:
    ...
    # 2. Transfer: Only scan departures within next 6 hours (up to 360 mins from start)
    max_dep_window = startTimeMin + 360 if startTimeMin > 0 else 1440
    max_hops = 3 if len(direct_results) == 0 else 1

    for d1 in orig_deps:
        if d1['depMin'] < startTimeMin: continue
        if d1['depMin'] > max_dep_window and len(transfer_results) >= 30: break
...
for (o, d) in [('板橋', '台北'), ('台北', '內灣'), ('內灣', '六家'), ('台中', '花蓮'), ('高雄', '台東')]:
    dr, tr, ms = ultra_fast_plan_routes(o, d, 300)
    print(f"[{o} -> {d}] Direct: {len(dr)}, Transfer: {len(tr)}, Speed: {ms:.2f}ms")
