...
// Implement the refined pruning logic:
sandbox.pruneDominatedRoutes = function(routes) {
    ...
    const validTransferRoutes = transferRoutes.filter(r => {
        ...
        for (let i = 1; i < r.legs.length; i++) {
            const nextTrainNo = r.legs[i].train_number;
            const nextTrain = sandbox.allTimetableData.find(t => t.train_number === nextTrainNo);
            if (nextTrain) {
                ...
                if (origStop) {
                    ...
                    // If the express train stopped at origin after the commuter train departed, passenger should just board the express directly!
    ...
    // Each distinct first train from origin (e.g. 103, 105, 107) is a legitimate departure choice.
    // Within the same first train (or same departure slot), keep the fastest onward connection.
    const nonDominatedTransfers = [];
    
    // Group routes by first leg train
    ...
    byFirstTrain.forEach((groupRoutes, firstTrainKey) => {
        // Within the same first train, prune dominated connections (e.g. later arrival or more transfers for the same first train)
