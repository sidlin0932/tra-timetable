...
def normalize_station(st):
    raw = str(st).replace('臺', '台').replace('\u3000', '').replace(' ', '').strip()
    raw_lower = raw.lower()
    if 'hualien' in raw_lower: return '花蓮'
    if 'taitung' in raw_lower: return '台東'
    if 'taipei' in raw_lower: return '台北'
    if 'kaohsiung' in raw_lower: return '高雄'
    if 'xincheng' in raw_lower: return '新城(太魯閣)'
    st = re.sub(r'[\d:\-－\s]+', '', raw)
    st = re.sub(r'[A-Za-z]+', '', st)
    st = st.replace('()', '').replace('（）', '')
    if st in ['新城太魯閣', '新城太魯閣']: st = '新城(太魯閣)'
    return st
...
def add_or_merge_train(t_num, t_type, t_model, is_tr, line, stops):
    ...
    if key not in all_trains_map:
        ...
        existing_stops = existing['stops']
        base_stops = list(existing_stops)
...
for fname, sheet_idx, line_name in commuter_files:
    ...
    try:
        ...
        for c in range(t_col + 1, len(df.columns)):
            ...
            if st in ['佳', '山佳']: st = '山佳'
            elif st in ['冬', '冬山']: st = '冬山'
            elif st in ['岡', '岡山']: st = '岡山'
            elif st in ['松', '松山']: st = '松山'
            elif st in ['南', '南港']: st = '南港'
            elif st in ['潮', '潮州']: st = '潮州'
            elif st in ['屏', '屏東']: st = '屏東'
            elif st in ['枋', '枋寮']: st = '枋寮'
            elif st in ['竹', '新竹']: st = '新竹'
            elif st in ['義', '嘉義']: st = '嘉義'
...
# 3. Mainline Express Trains (Western Trunk)
ENG_TO_ZH = {
...
for fname in western_express_files:
    ...
    try:
        ...
        for c in range(4, df.shape[1]):
            ...
            add_or_merge_train(t_num, t_type, t_model, is_tr, '西部幹線特快', stops)
    except Exception as e:
        print(f'Error parsing {fname}: {e}')

# 4. Eastern Mainline and South Link Express Trains
east_south_files = [
...
for fname, line_name in east_south_files:
    path = os.path.join(folder, fname)
    if not os.path.exists(path): continue
    try:
        xl = pd.ExcelFile(path, engine='odf')
        ...
        for r in range(13, len(df)):
            ...
            st = normalize_station(raw_st)
            if st and st not in ['nan', 'None', '站名']:
        ...
        for c in range(8, df.shape[1]):
            ...
            raw_type = str(df.iloc[type_row, c]).strip() if type_row < len(df) else ''
            raw_model = str(df.iloc[model_row, c]).strip() if model_row < len(df) else ''
            t_type, t_model, is_tr = extract_type_and_model(raw_type, raw_model)
            
            stops = []
            for r, st_name in stations:
                t_str = clean_time(df.iloc[r, c])
                if t_str:
                    stops.append({'station': st_name, 'time': t_str})
...
final_trains = sorted(list(all_trains_map.values()), key=lambda x: x['train_number'])

print(f"Master Database Rebuilt with proper line segregation! Total: {len(final_trains)} trains.")
