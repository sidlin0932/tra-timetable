...
def normalize_station(st):
    ...
    # Remove numbers and timestamps from header strings
    st = re.sub(r'[\d:\-－\s]+', '', st)
    return st
...
def add_or_merge_train(t_num, t_type, t_model, is_tr, line, stops):
    ...
    if t_num not in all_trains_map:
        ...
        base_stops.sort(key=lambda s: t_min(s['time']))
        existing['stops'] = base_stops
        existing['origin'] = base_stops[0]['station']
        existing['dest'] = base_stops[-1]['station']
...
# 2. Commuter Lines with Dynamic Header Extraction
commuter_files = [
...
for fname, sheet_idx, line_name in commuter_files:
    ...
    try:
        ...
        for c in range(t_col + 1, len(df.columns)):
            ...
            # Special case fix for station names
            if st in ['佳', '山佳']: st = '山佳'
...
print(f"Master Database Rebuilt with proper station normalization! Total: {len(final_trains)} trains.")
