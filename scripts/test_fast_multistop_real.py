...
// Fast single-pass segment planner for multi-stop waypoints
sandbox.planMultiStopRoutes = function(wps, startTimeMin) {
    ...
    // Segment 1 (A -> B)
    let candidateChains = sandbox.planRoutes(wps[0].station, wps[1].station, startTimeMin).slice(0, 20);
    ...
    for (let seg = 1; seg < wps.length - 1; seg++) {
        ...
        // Compute all segment routes ONCE
        ...
        for (let cIdx = 0; cIdx < candidateChains.length; cIdx++) {
            ...
            const viableNext = allSegRoutes.filter(r => sandbox.timeToMin(r.dep_time) >= earliestDep).slice(0, 2);
