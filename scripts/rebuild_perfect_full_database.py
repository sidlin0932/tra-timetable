...
print("Rebuilding perfect database with exact station start columns...")
...
# 2. Commuter Lines with Precise Starting Columns
commuter_specs = [
...
# 3. Western Trunk Express (Mountain / Sea)
...
# Deduplicate and merge multi-leg continuous trains
TRAIN_MAP = {}
for t in ALL_TRAINS:
    t_no = t['train_number']
    if t_no not in TRAIN_MAP:
        TRAIN_MAP[t_no] = t
    else:
        existing = TRAIN_MAP[t_no]
        ...
        if len(merged_stops) >= 2:
            ...
            if t.get('route_dir') and not existing.get('route_dir'):
                existing['route_dir'] = t.get('route_dir')

final_trains = sorted(TRAIN_MAP.values(), key=lambda x: int(x['train_number']) if x['train_number'].isdigit() else 99999)
print(f"Total Unique Trains: {len(final_trains)}")

t2007 = TRAIN_MAP.get('2007')
if t2007:
    print(f"Train 2007: {t2007['origin']} -> {t2007['dest']}")
    print("Stops of 2007:")
...
with open('data.js', 'w', encoding='utf-8') as f:
    f.write('window.EMBEDDED_TIMETABLE_DATA = ' + json.dumps(final_trains, ensure_ascii=False, indent=2) + ';')

print("Perfect database successfully rebuilt!")
